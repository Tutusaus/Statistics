...
pvalor<-1-pnorm(z)
# Com que el p-valor és 0.092>0.05
# acceptem H0
###################################
...
pvalor
# Com que en aquest cas el p-valor
# és menor pvalor<0.1 rebutjem H0
# i acceptem H1
###################################
...
pvalor<-2*(1-pnorm(z))
pvalor
# Com en aquest cas el p-valor>0.05
# nivell de significació acceptem H0.
# Si el nivell de significació fos
# alpha=0.1 considerariem H0 certa
# igualment i doncs tindriem una
# certesa del 90% que la mitjana
# de les dades es 3.5.
###################################

###### AMB SIGMA DESCONEGUDA ######

########### EXERCICI 1 ############
load("dades2.RData")
dades2
t.test(dades2, mu = 12, alternative = "less")
# Per tant, acceptem H0. Amb 5% de
# significació, no tenim prou
# estadística per afirmar que la
# mitjana és inferior a 12.
###################################
...
# El p-valor=0.2425>0.1=alpha per tant
# suposem H0 certa i considerem que la
# mitjana és igual a 12.
####################################
...
alpha<-0.01
prop.test(N, n, alternative = "less", conf.level = 1-alpha, correct = FALSE)
# Per alpha=0.01 torna a passar el mateix
# i doncs no existeixen evidències significatives
# que la proporció sigui inferior a 0.5.
...
# Observem que en aquest cas com que el
# pvalor és 0.039>0.01 acceptem H0 i doncs
# podem acceptar que la proporció de
# treballadors amb acccés de radiació
# és 0.4.
...
# El pvalor per ambdos testos és 0.0437.
# Pel primer test on el nivell de significació
# és 0.05 acceptarem H1 mentre que amb el segon test
# on el nivell de significació és de 0.01 ens
# decidim a favor de H0.
####################################
...
# Obtenim un pvalor de 0.876>0.1 per
# tant ens decantem per H0 i afirmem
# amb una confiança del 90% que la
# variància és de 1.
####################################

# CONTRAST DE DUES MOSTRES NORMALS #
########### INDEPENDENTS ###########
# Suposem en tots els casos que tenim
# mostres X i Y normals. El quocient
# de les variàncies de dues mostres
# s'aproxima una F de Fisher. El test
# de comparació de dues variancies
...
# El pvalor obtingut és de 0.844>0.1 per tant
# acceptem H0 i considerem que les variancies
# entre totes dues mostres son la mateixa. Amb
# 10% de significació, no tenim evidències
# estadístiques per afirmar que hi ha diferències
# significatives entre les variancies.
####################################
####################################

# CONTRAST SOBRE COMPARACIÓ DE MITJANES #
#########################################
...
# El pvalor obtingut és de 0.087<0.1 que és
# el nivell de significació. Per tant rebutjem
# H0 i afirmem que existeixen evidències
# significatives que recolzen el fet que les
# mitjanes son diferents i per tant hi ha diferència
# entre els dos tractaments.
#########################################
#########################################
...
# Obtenim un pvalor de 0.9997>0.05
# per tant acceptem H0 i doncs tenim
# evidència per donar suport al fet
# que la proporció d'esquerrans és
# del 30%. Amb una significació de
# 0.1 obtenim les mateixes conclusions.
####################################
############# APARTAT C ############
...
# Obtenim un pvalor de 0.7597>0.03
...
A
B
# Construïnt l'interval de confiança
# veiem clarament que 10 no està dins
...
# Com que el p-valor obtingut és de
# 1.36e-15<0.05 podem afirmar amb una
# confiança del 95% que la diferència
# de mitjanes és diferent de 2.
####################################
####################################
####################################
####################################